        ...
        {Object.entries(details || {}).slice(0, 4).map(([key, val]) => (
          <div key={key} className="flex justify-between items-center">
            ...
            <span className="text-[10px] font-semibold text-slate-700 text-right max-w-[120px] truncate">
              {typeof val === "number" ? (typeof val === "number" && val % 1 !== 0 ? (Number(val) || 0).toFixed(2) : val.toLocaleString()) : String(val)}
            </span>
          </div>
        ))}
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className={`glass-card p-5 ${data?.status === "operational" ? "border-green-200 bg-green-50/30" : "border-red-200 bg-red-50/30"}`}>
        <div className="flex items-center gap-3">
          ...
          <div className="ml-auto text-right">
            ...
            <p className="text-sm font-bold text-slate-700">{(Number(telemetry?.uptime_hours) || 0).toFixed(0)}h</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: "Avg Response", value: telemetry.avg_response_ms ? `${(Number(telemetry?.avg_response_ms) || 0).toFixed(0)}ms` : "—", icon: Clock, color: "text-green-600", bg: "bg-green-50" },
